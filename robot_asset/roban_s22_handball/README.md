...
左右手采用与 Kuavo 一致的两段式结构：

```text
zarm_l4_link + l_handball
zarm_r4_link + r_handball
```
...
本资产不采用将手与前臂合并为 `l_forearm_hand.STL` 或
`r_forearm_hand.STL` 的简化方案。`l_handball` 和 `r_handball` 是具有
质量和惯量的固定子 link，不是单纯的视觉占位符。
